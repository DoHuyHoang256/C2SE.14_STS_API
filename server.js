...
app.get('/api/users/email', (req, res) => {
  ...
  app.get('/api/users/email/:email', (req, res) => {
    ...
    pool.query('SELECT user_id, full_name, user_code, role, TO_CHAR(date_of_birth, \'DD-MM-YYYY\') AS date_of_birth, phone_number, address, email, gender, wallet FROM users WHERE email = $1', [email], (error, result) => {
        if (error) {
